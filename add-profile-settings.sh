...
print("Profile page created")

# Update dashboard layout
layout = open('app/dashboard/layout.tsx').read()

# Add UserCog to lucide imports if missing
if 'UserCog' not in layout:
    layout = layout.replace(
        'ShieldCheck, Bell, LogOut, Menu, X, User',
        'ShieldCheck, Bell, LogOut, Menu, X, User, UserCog'
    )
    print("Added UserCog import")

# Add profile to navItems
if '/dashboard/profile' not in layout:
    layout = layout.replace(
        "  { href: '/dashboard/invest', label: 'INVEST', icon: TrendingUp },\n]",
        "  { href: '/dashboard/invest', label: 'INVEST', icon: TrendingUp },\n  { href: '/dashboard/profile', label: 'PROFILE', icon: UserCog },\n]"
    )
    print("Added PROFILE to navItems")
...
git add . && git commit -m "add profile settings page: name, email, password" && git push
echo "Done!"
