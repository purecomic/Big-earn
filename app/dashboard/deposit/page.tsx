...
const WALLETS = [
  { name: 'Bitcoin (BTC)', address: 'bc1qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh', symbol: '₿', color: '#f7931a' },
  { name: 'USDT (TRC20)', address: 'TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE', symbol: '₮', color: '#26a17b' },
  { name: 'Ethereum (ETH)', address: '0x742d35Cc6634C0532925a3b8D4C9C5F9A1234567', symbol: 'Ξ', color: '#627eea' },
]
...
export default function DepositPage() {
  ...
  async function handleSubmit() {
    ...
    const { error: txError } = await supabase.from('transactions').insert({
      ...
      status: 'pending',
      description: `Deposit via ${WALLETS[selectedWallet].name}${txHash ? ` | TX: ${txHash}` : ''}`,
    ...
    await supabase.from('notifications').insert({
      ...
      message: `${user.full_name} requested a deposit of $${amount}`,
      type: 'info',
      is_broadcast: false,
    })
  ...
  if (success) return (
    <div style={{ padding: '40px 24px', textAlign: 'center', maxWidth: 400, margin: '0 auto' }}>
      ...
      <p style={{ color: 'rgba(255,255,255,0.5)', lineHeight: 1.7, marginBottom: 24 }}>
        Your deposit of <strong style={{ color: '#f5c842' }}>${amount}</strong> is being reviewed. 
        Your balance will be updated within 1–24 hours after confirmation.
      </p>
      <button className="btn-gold" onClick={() => { setSuccess(false); setAmount(''); setTxHash(''); setProofFile(null) }}>
    ...
    <div style={{ padding: '20px 16px', maxWidth: 500, margin: '0 auto' }}>
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.8rem', color: '#e8eaf0' }}>DEPOSIT FUNDS</h1>
        </div>
      ...
      <div style={{ background: 'rgba(96,165,250,0.08)', border: '1px solid rgba(96,165,250,0.25)', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', gap: 10 }}>
        ...
        <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.55)', lineHeight: 1.6 }}>
          Send crypto to the address below, then submit the form with your transaction hash. Admin will verify and credit your account within 24 hours.
        </p>
      </div>
      ...
      <div className="card" style={{ padding: '20px', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {WALLETS.map((w, i) => (
            <button key={i} onClick={() => setSelectedWallet(i)} style={{
              display: 'flex', alignItems: 'center', gap: 12, padding: '14px',
              background: selectedWallet === i ? `${w.color}15` : 'rgba(255,255,255,0.03)',
              border: `1px solid ${selectedWallet === i ? w.color + '60' : 'rgba(255,255,255,0.08)'}`,
              borderRadius: 12, cursor: 'pointer', transition: 'all 0.2s'
            }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: `${w.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', fontWeight: 700, color: w.color }}>
                {w.symbol}
              </div>
      ...
      <div className="card" style={{ padding: '20px', marginBottom: 16 }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(245,200,66,0.2)', borderRadius: 10, padding: '14px 16px', wordBreak: 'break-all', fontSize: '0.85rem', color: '#f5c842', lineHeight: 1.6, marginBottom: 12 }}>
      ...
      <div className="card" style={{ padding: '20px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px', background: 'rgba(255,255,255,0.04)', border: '1px dashed rgba(245,200,66,0.25)', borderRadius: 10, cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: '0.85rem', color: proofFile ? '#f5c842' : 'rgba(255,255,255,0.35)' }}>
                {proofFile ? proofFile.name : 'Tap to upload proof'}
              </span>
        ...
        <button className="btn-gold" onClick={handleSubmit} disabled={submitting} style={{ width: '100%', marginTop: 20, opacity: submitting ? 0.7 : 1 }}>
