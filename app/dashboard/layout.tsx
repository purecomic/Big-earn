    ...
    <div style={{ minHeight: '100vh', background: '#050810', display: 'flex', flexDirection: 'column' }}>
      ...
      <header style={{
        ...
        >
          <Bell size={22} />
          {unreadCount > 0 && (
            <span style={{ position: 'absolute', top: 4, right: 4, width: 16, height: 16, borderRadius: '50%', background: '#f5c842', color: '#050810', fontSize: '0.6rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {unreadCount}
            </span>
          )}
        </button>
      ...
      {notifOpen && (
        <>
          ...
          <div style={{
            position: 'fixed', top: 60, right: 0, left: 0, maxWidth: 400, margin: '0 auto',
            background: '#0a0f1e', border: '1px solid rgba(245,200,66,0.15)',
            borderRadius: '0 0 16px 16px', zIndex: 50, maxHeight: '70vh', overflowY: 'auto'
          }}>
            <div style={{ padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(245,200,66,0.1)', position: 'sticky', top: 0, background: '#0a0f1e' }}>
              <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', color: '#e8eaf0' }}>NOTIFICATIONS</span>
              <button onClick={markAllRead} style={{ background: 'none', border: 'none', color: '#f5c842', fontSize: '0.75rem', cursor: 'pointer' }}>Mark all read</button>
            </div>
            {notifications.length === 0 ? (
              <div style={{ padding: '24px', textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: '0.9rem' }}>No notifications yet</div>
            ) : notifications.map(n => (
              <div key={n.id} style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)', background: n.read ? 'transparent' : 'rgba(245,200,66,0.04)' }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', marginTop: 5, flexShrink: 0, background: n.type === 'success' ? '#4ade80' : n.type === 'warning' ? '#f5c842' : n.type === 'alert' ? '#f87171' : '#60a5fa' }} />
                  <div>
                    ...
                    <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)', marginTop: 6 }}>{new Date(n.created_at).toLocaleDateString()}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {sidebarOpen && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex' }}>
          <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 280, background: '#0a0f1e', borderRight: '1px solid rgba(245,200,66,0.15)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
            <div style={{ padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid rgba(245,200,66,0.1)' }}>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem' }}>
                <span className="gold-text">BIG</span><span style={{ color: '#e8eaf0' }}> EARN</span>
              </div>
              <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', color: '#e8eaf0', cursor: 'pointer' }}><X size={20} /></button>
            </div>
            ...
            <nav style={{ flex: 1, padding: '12px 0' }}>
              {navItems.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href} style={{ textDecoration: 'none' }} onClick={() => setSidebarOpen(false)}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', background: active ? 'rgba(245,200,66,0.08)' : 'transparent', borderLeft: active ? '3px solid #f5c842' : '3px solid transparent', color: active ? '#f5c842' : 'rgba(255,255,255,0.6)' }}>
                      <Icon size={18} />
              ...
              {isAdmin && (
                <div>
                  <button onClick={() => setAdminUnlock(!adminUnlock)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '14px 20px', background: 'none', border: 'none', color: 'rgba(248,113,113,0.8)', cursor: 'pointer', textAlign: 'left' }}>
                    <ShieldCheck size={18} />
                    <span style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', letterSpacing: '0.05em' }}>ADMIN PANEL</span>
                    <span style={{ marginLeft: 'auto', fontSize: '0.65rem', background: 'rgba(248,113,113,0.2)', border: '1px solid rgba(248,113,113,0.4)', color: '#f87171', padding: '2px 8px', borderRadius: 100 }}>ADMIN</span>
                  </button>
                  {adminUnlock && (
                    <div style={{ margin: '0 16px 12px', padding: '14px', background: 'rgba(248,113,113,0.07)', border: '1px solid rgba(248,113,113,0.2)', borderRadius: 12 }}>
                      ...
                      <input className="input-field" type="password" placeholder="Password" value={adminPassInput} onChange={e => setAdminPassInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAdminAccess()} style={{ marginBottom: 10, fontSize: '0.9rem', padding: '10px 14px', borderColor: adminPassError ? 'rgba(248,113,113,0.6)' : undefined }} />
            ...
            <div style={{ padding: '16px 20px', borderTop: '1px solid rgba(245,200,66,0.08)' }}>
              <button onClick={() => { signOut(); router.push('/') }} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', cursor: 'pointer', fontSize: '0.9rem' }}>
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} style={{ flex: 1, textDecoration: 'none' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 4, color: active ? '#f5c842' : 'rgba(255,255,255,0.35)', position: 'relative' }}>
                {active && <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 32, height: 2, background: '#f5c842', borderRadius: '0 0 2px 2px' }} />}
                <Icon size={20} strokeWidth={active ? 2.5 : 1.8} />
                <span style={{ fontSize: '0.6rem', fontWeight: active ? 600 : 400, letterSpacing: '0.05em' }}>{item.label}</span>
              </div>
            </Link>
          )
        })}
